    ...
    public class LookRotationOneAxis : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            else
            {
                if (hasAddRotation)
                {
                    var addRotation = Quaternion.Euler(new Vector3(-lookTarget.eulerAngles.x, rotation.eulerAngles.y,
                        rotation.eulerAngles.z));
                    rotation = Quaternion.Lerp(rotation, addRotation, lerpFactorCallibration);
                }

                transform.rotation = Quaternion.Slerp(transform.rotation,
                    new Quaternion(rotation.x, rotation.y, rotation.z, rotation.w), Time.deltaTime * damping);
            }
        }
    }
}
